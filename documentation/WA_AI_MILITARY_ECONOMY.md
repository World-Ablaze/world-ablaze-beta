...
| E4 | `put_unit_buffers` | Single-block `ratio` ≤ **0.25**. No `enable = { always = yes }`. `subtract_fronts_from_need = no` requires a `# pool:` comment in the block explaining why this buffer must not yield to front demand (cf. the Atlantic Wall lesson). Exemptions (a scripted siege/festung state) require a `# siege:` comment and remain ≤ 1.0. |
| E5 | `force_concentration_factor` / `_front_factor` / `_target_weight` | Written **only** by `WA_AI_MILITARY_DEFAULT_FRONT_aifc.txt`. Boost and paired suppression must share the same gating (same `enable`, same `ratio` gate or none on both). |
...
| 7b | Unambiguous defect fixes valid under any economy: `has_variable` guards on `WA_AI_fielded_eq_ratio` readers (done 2026-08-09), AIFC gating seams, no-op define cleanup. | in progress |
