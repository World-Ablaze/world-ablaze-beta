...
| E4d | `put_unit_buffers` theatre share cap | A garrison FAMILY (several blocks over one theatre for one country) may carry a **share cap**: a twin family at reduced ratios, armed by an `X` / `NOT X` pair on one observation trigger so the two never co-fire (`[usa-pacific-hoard]`: `WA_AI_MILITARY_is_two_ocean_war_with_large_army`, latched one-way on `constant:wa_ai_theatre.usa_pacific.cap_min_div`). **Counting rule for E4c:** a family whose gates are provably exclusive on one trigger counts its LARGEST member, not the sum of both — the USA Pacific file reads full 0.54 + skeleton 0.50 (pre-existing co-fire below 110 divisions, outside the subject) against capped 0.31 non-yielding, so the file's worst case is unchanged by the twin. The cap is a **nominal** ratio sum (0.49); the owner's bound is on the realised share, and realised ≤ nominal is ASSUMED (campaign `5d2a391c` read 0.5–0.7 × nominal) — the save-side probe is buffer divisions / `num_divisions` ≤ 0.50 on every two-ocean save (`plans.py TAG <save>`). |
