...
| E4b | `put_unit_buffers` pool coherence | Entries sharing an `order_id` share **one ratio pool** (`common/ai_strategy/documentation.info:194-225`; the 2026-08-09 Atlantic Wall lesson). All members of a pool must therefore agree on `subtract_fronts_from_need` and on whether they declare an `area` — otherwise one member's flag silently decides for states it was never written for. A garrison that must hold against front demand gets **its own** `order_id`, not a shared one. |
