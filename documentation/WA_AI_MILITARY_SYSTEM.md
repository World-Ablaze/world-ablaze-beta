...
| `force_concentration_factor` | **I** - Sums, added to define `AIFC_UNIT_RATIO_BASE` (0.15) | Additive | n/a | -100 (hard off) to +35, in percentage points; Layer 1 is one flat +35 (50 % for every country, `[aifc-flat-share]`) |
...
| Consumers (AIFC) | `WA_AI_AIFC_posture_defensive` reads the `WA_AI_AIFC_hold_the_line` flag (the former `WA_AI_AIFC_posture_offensive` +8 bonus was retired under `[aifc-flat-share]`) |
