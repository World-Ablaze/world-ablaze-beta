...
> **Landing note for the r103 / r104 pair — delete once both halves are in.** Fix 103 (v25) and
> Fix 104 (v26) split the same judgement — *permission* vs *payment* — and were built in parallel by
> two sessions, so they commit separately and in either order. Consequences while only one is in:
> the `@WA_TLM_VERSION` bump travels with whichever lands **last**, so a build can carry the `r104_*`
> variables and still report an older version — **score on the variables' presence, not on the version
> number**; and the `r103_corridor_orphan` gauge that the R69 row names as its paired detector is
> simply absent until the other half lands, which is not a failure of either fix.
