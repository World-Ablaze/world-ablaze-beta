...
| `WA_TLM_pc_stale_kept_n` | counter | **at the keep decision** in `WA_AI_PC_validate_queued_provincial_projects` — an off-path project kept because it is paid past `constant:wa_ai_pc.alloc.stale_keep_paid_fraction` | standing — the other half of the validator's verdict ([rail-admission-churn]). NOT a departure: read `pc_stale_n + pc_stale_kept_n` as "off-path projects judged", `pc_stale_n` alone as "cancelled". A kept_n stuck at 0 while stale_n climbs means the fraction never fires | v35 |
...
| `WA_TLM_sq_adds_n` | counter | `WA_AI_add_<X>` (building_adders), all nine shared-slot types, only when `WA_AI_available_<X>` passed and the add was issued | standing — family **sq** (standard construction queue); R49, R53 | v17 |
| `WA_TLM_sq_adds_by_type^i` | counter (indexed 0 CIC · 1 MIC · 2 NIC · 3 REF · 4 SR · 5 HSR · 6 AR · 7 HAR · 8 SILO; not zero-initialised, `sq_adds_n` is the witness) | same site | standing — per-type split of the above | v17; ^8 v31 |
...
| `WA_TLM_post_exec_n` | counter | **weekly, per (country, enemy)** at the posture calculus (`WA_AI_MILITARY_posture_effects.txt`, inside `every_enemy_country`) — NOT the monthly sampler | posture execute-level (levels 1 and 3 since `[posture-v3]`) exchange-rate question | v6 |
| `WA_TLM_post_exec_xr_n` | counter | same site, execute-level (1 and 3 since `[posture-v3]`) observations that had a usable ratio | **denominator for the three buckets below** | v6 |
...
| `WA_TLM_r114_thair_near_n` / `_far_n` | counter pair | **at the verified start** in `WA_AI_build_theatre_air_bases` — same site as `r52_thair_front_n`, one increment per air-base project actually appended, split by which PHASE of the Fix 114 ladder was in force: `near` = the contested-edge phase (`WA_AI_PC_state_is_near_contested_edge`), `far` = the whole-theatre fallback. `_first_t` / `_last_t` stamp the `near` half only | R78 — **is the theatre air force based where the fighting is.** Read as a RATIO, never either half alone: `far_n` is not a failure counter (a theatre contested at a distance is supposed to land in the rear) and `near_n` alone cannot separate "the phase works" from "the fallback never had to run". Second signal, and the one that matters: `airload.py --states <theatre state ids>` must show the front-line member states gaining nominal capacity against the rear. Counters up and the map flat = the projects are queued and not funded — go to `pc TAG --match air_base` | v29 |
...
| `WA_TLM_r103_corridor_blocked_n` | counter | `WA_AI_PC_start_railway_project` (`railway_helpers.txt`), corridor family only and **corridor 1 only** ([levant-iraq]), **placed after the `current_level < target` check** — one per corridor segment that wanted a level and whose state failed `WA_AI_PC_state_controller_allows_admission`. Placement is the definition: before the level check, every already-built hop with a hostile-state end would book a refusal that never happened. Calls the same trigger the executor calls, so probe and gate cannot drift | R68 — **makes the silent refusal visible.** This is the class that hid the *second* dead hop of `7c7803a8` (7079 ↔ 5078) for 28 months: an ally-held province inside an enemy-controlled state, requested and pathfound every 4 weeks, never queued, no trace. `blocked_n` climbing while `seg_n` is flat = the province/state scope mismatch, not a selector problem | v25 |
