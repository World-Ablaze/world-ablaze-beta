...
| `WA_TLM_r48_c10_fire_n` | counter | **top of the `WA_AI_C.10` immediate block** (`events/WA_AI_construction.txt`), every dispatch of the standard-construction event. NOT the monthly sampler | R48 (Fix 84) — **cadence of the whole standard construction event per country**; C.10 is dispatched daily behind an idle-civ throttle plus its own 75–1000-day re-arm timer, so a major reading single digits over a decade is a country whose vanilla queue never empties. Denominator for `_c10_escape_n` | v16 |
| `WA_TLM_r48_c10_escape_n` / `_c10_escape_last_t` | counter + stamp | same site, only when `num_of_available_civilian_factories` ≤ 1 at dispatch — which, by construction of the on_daily gate, can only be the pre-build escape | R48 (Fix 84) — **separates the two silences.** `escape_n = 0` with `prebuild_synth_n = 0` = the dispatch never reached C.10 (trigger terms or the gate); `escape_n > 0` with `prebuild_synth_n = 0` = it reached C.10 and `WA_AI_queue_REF` found no free core site (`any_of_scopes` precondition). `escape_n` climbing after the country's rubber balance turned negative is impossible by the trigger's own terms — if seen, the trigger's `resource@rubber` read is not what the ledger says | v17 |
...
| `WA_TLM_r54_air_headroom_civs` / `_first_t` / `_last_t` | counter (civ-weeks) + stamps | at both air-lane grants, only the part of the grant filled from HEADROOM (beyond the `@AI_PC_ALLOC_FRACTION` cut, up to the 0.50 hard cap), latched per pass | R54 (P2) — **the measured cost of the fix**: every unit is one civ-week the air lane took out of the vanilla construction queue rather than out of another PC project. Read against `wa_tlm_pc_civs_avail` (the idle margin it should be eating first) and against the vanilla queue's own output (`wa_tlm_sq_*`, mil/civ factories, USA synthetic refineries). Large on a country with `civs_avail = 0` (ENG 1945) means it is displacing real vanilla builds, not idle civs | v17 |
