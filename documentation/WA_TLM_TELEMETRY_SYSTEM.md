...
| `WA_TLM_r74_ally_rail_routes_n` | counter | **at the route-accept site** in `WA_AI_PC_railway_land_consider_frontline` (`WA_AI_CONSTRUCTION_PRIORITY_railway_strategies.txt`) — one per land-war route appended to `railway_end_provinces_`, NOT on entry to the effect, so a candidate that failed pathfinding is not counted. Not the monthly sampler | R46 — **denominator.** Separates "this country queues rail at all" from "the ally leg fired"; ENG/USA read 0 for the whole of campaign `f9321934` | v11 |
...
| `WA_TLM_r97_ea_states_manned` | gauge | monthly, `WA_TLM_sample_east_africa` (`WA_TLM_core.txt`), only on an AI country for which the East-African theatre is contested (`WA_AI_MILITARY_east_africa_theatre_contested`) or that holds the colony (`_holds_east_africa_colony`) | R62 — **the verified effect of Fix 97**: of the 19 theatre states (list = `WA_AI_MILITARY_AIR_theatre_contested_east_africa`), how many hold ≥ 1 of the country's OWN divisions. `0edbc955` baseline: ENG 2-6, ITA up to 8 (inside enemy states). Never read `_contested` alone | v21 |
| `WA_TLM_r97_ea_contested` | gauge (0/1) | same site | R62 — code-path companion: the Faction pull's gate as this country reads it | v21 |
| `WA_TLM_r97_ea_first_t` / `_last_t` | stamps | same site; `_first_t` under a `= 0` guard the first sampled month with `_contested = 1` | R62 — timing; read `_last_t > 0` before trusting a 0 | v21 |
...
| `WA_TLM_r65_aifc_arm_entries_n` | counter | **inside each of the four emitters** (`WA_AI_AIFC_armor_add_boost` / `_cancel_boost` / `_add_suppress` / `_cancel_suppress`, `WA_AI_AIFC_helpers.txt`), one per `add_ai_strategy` issued. WEEKLY, every AI country the reconcile reaches — NOT the monthly sampler | R65 — the save bloat, self-reported. **Counts emitter INVOCATIONS, not confirmed engine entries**: script cannot read back the `persistent_strategy` list, so honesty rule 1 is met only as far as the engine allows. **Second signal, and it is a strong one**: the save's own type-83 count per major, which is directly countable — baseline USA **517** at 1944.6 on campaign `7c7803a8` (ITA 357, GER 219, ENG 84, JAP 38, FRA 18). A campaign whose `entries_n` and type-83 count diverge means an emitter is failing silently (the meta_effect lesson) | v24 |
| `WA_TLM_r65_aifc_arm_retire_n` | counter | **at the head of `WA_AI_AIFC_armor_reconcile`**, on a week entered with no sector enemy AND a non-empty `WA_AI_AIFC_armor_suppressed` — i.e. exactly the weeks section 2 wipes the book | R65 — **episode COUNT.** Once per episode by construction: a second consecutive lapsed week finds the book already empty and books nothing | v24 |
| `WA_TLM_r65_aifc_arm_install_n` | counter | end of the same effect, on the verified transition (book empty at entry, non-empty at exit) | R65 — episode CLOSE. `install_n − retire_n ∈ {0,1}`; 1 means a book is currently installed | v24 |
| `WA_TLM_r65_aifc_arm_lapse_wk` | counter | head of the same effect, every week entered with `_aifc_want < 1` | R65 — **the whole point of the family.** `lapse_wk / retire_n` = **mean episode length in weeks**, which is the number a retirement grace window must be sized against and which the entry count *cannot* supply: each episode emits one retire + one reinstall regardless of length, so 517 entries cannot distinguish 8 short lapses from 8 long ones, and monthly saves cannot resolve a weekly cadence. Note `_aifc_want < 1` conflates the transient no-anchor case with deliberate switch-off / war end / ineligibility — read it against `wa_ai_aifc_sector_anchor` and the country's war state before sizing anything | v24 |
| `WA_TLM_r65_aifc_arm_first_t` / `_last_t` | stamps | same effect, written only on ticks that actually emitted | R65 — persistence. `_first_t` under a `= 0` guard, so read `entries_n > 0` first. A frozen `_last_t` on a healthy tag is the **expected** reading (ENG's reconcile did zero work for 24 months), not an alarm — pair it with `retire_n = 0` to tell health from a dead code path | v24 |
