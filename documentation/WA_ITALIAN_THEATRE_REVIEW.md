...
- **AIFC / schwerpunkt selection is NOT the cause. Measured and refuted 2026-08-12.** Italy *was* the Allied schwerpunkt for most of the war: USA anchored in Italian states (regions 21/23/238) for **10 consecutive months** 1943.8–1944.5; ENG for **22 of 32 saves**, including an unbroken **15-month run** 1945.1–1946.3 that is still live at the final save. `sector_age` cycles healthily 1–4 on both (note the real ceiling is `constant:wa_ai_aifc.sector.max_age = 4` in `common/script_constants/wa_ai_aifc.txt`, not the 1–5 R1 quotes). The theoretical argument that narrow fronts and mountain terrain (`WA_AI_AIFC_helpers.txt:361-368`, mountain −50) should exclude Italy is **wrong in practice** — do not rebuild it.
...
- **Elasticity.** Germany's `put_unit_buffers` `ratio` is a fraction of the **total army** (`common/ai_strategy/documentation.info section put_unit_buffers`), so its Italian commitment scales with the Wehrmacht. The Allies have **zero `put_unit_buffers` in Italy**; the only army-fraction reserve the USA owns points at the Pacific (`COUNTRY_USA_THEATRE.txt:216-338`, order 9101, `subtract_fronts_from_need = no` — a hard reserve that does not yield to front demand).
...
- **Is `target =` a valid selector for `front_unit_request`?** `documentation.info section front_unit_request / invasion_unit_request` lists only `tag / state / strategic_region / area / country_trigger / state_trigger`; `target =` is the `build_building` selector (`ENG.txt:50-55`). If it is silently dropped, then **all of Canada's European weighting** (`COUNTRY_CAN_FRONT.txt:27-66`), `ALLIES_focus_on_europe_post_france` (`FACTION_ALLIES_FRONT.txt:334-373`), `ENG_gang_up_GER_with_USA_FRONT`, `USA_gang_up_GER_with_ENG` and `ENG_war_against_ITA_3_DIPLOMACY` contribute nothing. CAN never appears in Italy in any month of `9a4cd657`, which is consistent. **Cheap to test and affects far more than Italy.**
- **Does `front_unit_request tag = GER` reach ITA/RIT-owned provinces?** `documentation.info section front_unit_request / invasion_unit_request` glosses `tag =` as "province of a specific country". If that reading is right, `ALLIES_europe_first`'s +150 — the Allies' single largest positive European pull — never reaches the Italian peninsula at all, while every negative is region-keyed and does.
...
1. ~~**Untouched but adjacent:** the Sicily predicate is what retires ENG's North African `order_id = 1` buffer and `ENG_defense_of_el_alamein`.~~ **SUPERSEDED by Fix 102 (2026-08-17).** The Sicily predicate retired nothing: in campaign `7c7803a8` the Allies never took Sicily (115/1032 ITA-owned *and* ITA-controlled at 1943.11), so both blocks were `enable`-TRUE for the whole campaign. `WA_AI_MILITARY_ENG_protect_africa` was **starved**, not retired — yielding (`subtract_fronts_from_need` at its default) over `area = north_africa`, the one area holding ENG's entire land war, so front demand consumed its need; it produced an order in 1 of 17 monthly samples (instances 1817/1822, 1942.11.23–24, one division each). It is now deleted and its states folded into `ENG_defense_of_el_alamein`, non-yielding on a private `order_id`. The same defect was measured on `ITA_protect_libya_THEATRE` and both `ALLIES_commonwealth_*` North African blocks and fixed in the same commit.
...
Cause 3 (the RIT support stack — supply immunity, uncapped resupply, the ≈ −2500 anti-invasion suppression) and cause 4 (German bidding asymmetry). Plus every item in §8, of which the `target =` selector question is now **answered**: `documentation.info section front_unit_request / invasion_unit_request` lists the accepted set as `tag / state / strategic_region / area / country_trigger / state_trigger`, so `target =` is invalid and Canada's entire European weighting is presumed inert. That is its own commit and affects far more than Italy.
