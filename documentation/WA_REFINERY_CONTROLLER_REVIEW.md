...
- **No variable, trigger or metric anywhere exposes "N of my refineries are off for want of input."** The counter family (`total_*`, `current_*`) is no longer frozen for AI countries: the three `recalculate_*` effects were moved out of the `is_ai = no` gate in `on_weekly`, so the counts now track for every existing country and the refinery panel reads true under `observe`. **`open_*_target` still has no AI writer**, which is deliberate — it is the single variable in this system that expresses *intent* rather than realised capacity, and pinning it to `current_*` for AI countries would make Q5's demand calculation return zero by construction. The panel hides the column for AI rather than print the stale value. `manage_*` and `calculate_resource_deficite` stay `is_ai = no`, including on the GUI's Refresh button: they are a second refinery controller (`resource@iron < 0`) and would fight the AI missions (`< -26`).
...
5. **Should the player and AI paths be unified?** They are the same system split by `is_ai`, and neither half is complete: the AI has decisions with no bookkeeping, the player has bookkeeping wired only to a GUI. The player half already computes demand from *intended* capacity (`calculate_resource_deficite`) — the one calculation the AI path lacks.
   *Status note:* the AI-side bookkeeping half is now partly closed — `total_*` and `current_*` track for every country. What Q5 still needs is the missing half: a real `open_*_target` written by the AI controller at the moment it bids, not a mirror of `current_*`.
