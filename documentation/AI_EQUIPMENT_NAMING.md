...
| `__qualifier` | Only when the group carries **two designs on the same type** (151 designs today: module-tier steps on one airframe, cutting-corners twins). Double underscore, never present in a type name (MEASURED 0/1631). Closed vocabulary below (a compound word such as `cc_wa` is one vocabulary entry), else an ordinal `v2`, `v3`, … in file order; the first design on a type keeps the bare key. |
...
| `wa` / `la` / `wa_la` | steel-grade / ammunition-grade twins of `[resource-grade-downshift]`: weakened armour, non-APCR shell, both - gated by `WA_AI_EQUIPMENT_should_mount_weak_armor` / `_low_ammo`; GENERATED by `tools/migrations/grade_pairs/gen_grade_pairs.py`, never hand-edited |
| `cc_wa` / `cc_la` / `cc_wa_la` | the same twins of a `__cc` design |
