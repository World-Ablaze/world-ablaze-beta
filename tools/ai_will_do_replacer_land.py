...
CATEGORY_TO_TRIGGER = {
    ...
    "heavy_artillery": "WA_AI_RESEARCH_needs_heavy_artillery",
...
def get_special_forces_type(triggers: list[str]) -> Optional[str]:
    ...
    for trigger in triggers:
        if 'paratrooper_horse_battalion_line' in trigger.lower():
            sf_types.append('paratroopers')
        elif 'marine_horse_battalion_line' in trigger.lower():
        ...
        for child_tech in forward_graph[tech_name]:
            ...
            if current_sf_type is not None:
                ...
                for trigger in child_triggers:
                    ...
                    if current_sf_type == 'paratroopers' and 'paratrooper_horse_battalion_line' in trigger_lower:
                        filtered_child_triggers.add(trigger)
                    elif current_sf_type == 'marines' and 'marine_horse_battalion_line' in trigger_lower:
                        filtered_child_triggers.add(trigger)
                    ...
                    elif 'paratrooper_horse_battalion_line' not in trigger_lower and 'marine_horse_battalion_line' not in trigger_lower and 'mountaineer' not in trigger_lower:
                        filtered_child_triggers.add(trigger)
...
def determine_special_forces_types(tech_name: str, categories: list[str], dependencies: list[str]) -> list[str]:
    ...
    for cat in categories:
        if cat in CATEGORY_TO_TRIGGER:
            ...
            if 'special_forces' in trigger.lower() or 'paratrooper_horse_battalion_line' in trigger.lower() or 'marine_horse_battalion_line' in trigger.lower() or 'mountaineer' in trigger.lower():
    ...
    if not triggers:
        for pattern, trigger in TECH_NAME_PATTERNS:
            if 'paratrooper_horse_battalion_line' in trigger.lower() or 'marine_horse_battalion_line' in trigger.lower() or 'mountaineer' in trigger.lower():
    ...
    if file_path:
        ...
        if filename == 'special_forces_doctrine.txt':
            if re.search(r'paratrooper|paratroopers|paras_', tech_name, re.IGNORECASE):
                return "WA_AI_RESEARCH_needs_paratroopers"
            elif re.search(r'marine_horse_battalion_line|marines', tech_name, re.IGNORECASE):
                return "WA_AI_RESEARCH_needs_marines"
