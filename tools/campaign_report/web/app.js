...
async function boot(text) {
  ...
  const S = {tags:new Set(D.default_tags.filter(t => allTags.includes(t))),from:0,to:snapshots.length-1,at:snapshots.length-1,tab:"overview",force:"land",scope:"controlled",resource:"steel",armor:"all",percent:false,indexed:false,hidden:new Set(),convoy:"month"};
  const tabs = {overview:["Overview","Compare capabilities, follow their evolution, and identify pressures to investigate."],forces:["Forces","Compare force sizes and inspect the composition behind the totals."],industry:["Industry & resources","Track installed industry and the balance of resources."],armor:["Armor","Connect stockpiles, recorded requests, and production lines."],wars:["Wars & casualties","Follow casualties in the context of the wars included in each observation."],country:["Country status","Stability, war support, command power, and available experience."]};
  ...
  function armorValues(country){const fs=country?.armor?.families;if(!fs)return [];return S.armor==="all"?Object.values(fs):fs[S.armor]?[fs[S.armor]]:[];}
  function armorSum(country,key){if(!country?.armor)return null;const values=armorValues(country);if(values.length)return sumKnown(values.map(v=>v[key]));if(["production_per_day","training_need","deficit"].includes(key))return null;return finite(country.metrics?.[["stock","stock_deficit","active_factories"].includes(key)?"aircraft_stock":"divisions"])?0:null;}
  function armor(){const el=$("content"),bar=controlBar(),keys=[...new Set(snapshots.slice(S.from,S.to+1).flatMap(s=>tags().flatMap(t=>Object.keys(s.countries[t]?.armor?.families || {}))))].sort();
    if(S.armor!=="all"&&!keys.includes(S.armor))S.armor="all";
    const label=document.createElement("label");label.innerHTML=`Family <select aria-label="Armor family"><option value="all">All tanks and derivatives</option>${keys.map(k=>`<option value="${esc(k)}" ${k===S.armor?"selected":""}>${esc(labelType(k))}</option>`).join("")}</select>`;label.querySelector("select").onchange=e=>{S.armor=e.target.value;render();};bar.append(label);
    el.append(notice("Stockpiles are signed. Reinforcement requests are recorded by compatible family; they cannot be attributed to individual variants. Shortfall = recorded reinforcement requests - stock (DERIVED): requests may include equipment already in transit, so it is pressure, not a certified shortage. Training requirement and daily output are not computed.",true));
    ...
    const foreignLines=tags().map(tag=>{const vs=(current(tag)?.armor?.variants || []).filter(v=>S.armor==="all"||v.family===S.armor);const total=sumKnown(vs.map(v=>Math.max(0,v.stock ?? 0))),foreign=sumKnown(vs.filter(v=>v.creator&&v.creator!==tag).map(v=>Math.max(0,v.stock ?? 0)));return total>0&&foreign/total>=.5?`${labelCountry(tag)}: ${fmt(foreign)} of ${fmt(total)} in stock (${Math.round(foreign/total*100)} %) were built by other countries (captured or received), factories assigned ${fmt(armorSum(current(tag),"active_factories"))}.`:null;}).filter(Boolean);
    if(foreignLines.length)el.append(notice(`Foreign-built stock at ${dateText(times[S.at])} — ${foreignLines.join(" ")}`));
    const armorMeta=(key,unit="equipment")=>({unit,evidence:"DERIVED",source:key==="stock"?"production/equipments → variant registry":key==="active_factories"?"production/military_lines":`units/division → armor/${key}`,note:"Aggregated by equipment-definition family."});
    const g=grid();for(const [key,title,unit] of [["stock","Armor stockpile","equipment"],["deployed","Armor in divisions","equipment"],["reinforcement_need","Recorded reinforcement requests","equipment"],["active_factories","Factories assigned to armor","factories"]]){
      const meta=armorMeta(key,unit);g.append(chart(title,countriesSeries(c=>armorSum(c,key),meta),unit,{note:meta.note}));
    }
    // Factories a line asks for versus factories the engine actually assigned to it, per country.
    // The save carries both numbers per production line; the gap covers every cause at once
    // (factories not yet available, damage, priority), none of which is attributable from a save.
    ...
    const linesSum=(c,key)=>{if(!c?.armor)return null;return (c.armor.variants || []).filter(v=>S.armor==="all"||v.family===S.armor).flatMap(v=>v.production_lines||[]).reduce((a,l)=>a+(l[key]||0),0);};
    ...
    const shortfall=c=>{const need=armorSum(c,"reinforcement_need"),stock=armorSum(c,"stock");return finite(need)&&finite(stock)?Math.max(0,need-stock):null;};
    ...
    tags().forEach(tag=>{const series=[["stock","Stock (signed)",armorMeta("stock")],["reinforcement_need","Recorded reinforcement requests",armorMeta("reinforcement_need")],["shortfall","Shortfall",shortfallMeta]].map(([key,name,meta],i)=>({name,color:palette[i],meta,points:points(s=>key==="shortfall"?shortfall(s.countries[tag]):armorSum(s.countries[tag],key))}));sd.append(chart(labelCountry(tag),series,"equipment",{indexable:false,note:shortfallMeta.note}));});
    el.append(heading("Variants at the inspection date",dateText(times[S.at])));
    const variants=tags().flatMap(tag=>(current(tag)?.armor?.variants || []).filter(v=>S.armor==="all"||v.family===S.armor).map(v=>({tag,...v})));
    ...
    const redraw=()=>{const query=search.value.toLowerCase();const filtered=variants.filter(v=>(`${variantName(v)} ${v.name} ${v.definition} ${v.creator} ${v.family}`).toLowerCase().includes(query)&&(origin.value==="all"||(origin.value==="domestic"?v.creator===v.tag:v.creator&&v.creator!==v.tag)));holder.replaceChildren(tablePanel(["Country","Variant","Family","Creator","Stock","Deployed","Factories"],filtered.map(v=>[countryCell(v.tag),`${esc(variantName(v))}<small>${esc(v.definition || "")}</small>`,esc(labelType(v.family)),esc(v.creator || "—"),fmt(v.stock,1),fmt(v.deployed,1),fmt(v.active_factories)])));if(!filtered.length)holder.append(notice("No variants in this selection."));};search.oninput=redraw;origin.onchange=redraw;redraw();
    const rawButton=document.createElement("button");rawButton.textContent="Inspect recorded production lines";rawButton.onclick=()=>showTable("Recorded armor production lines",["Country","Variant","Active factories","Requested factories","produced (raw)","speed (raw)","cost (raw)"],variants.flatMap(v=>(v.production_lines||[]).map(line=>[labelCountry(v.tag),variantName(v),line.active_factories,line.requested_factories,line.produced,line.speed,line.cost])),`${snapshots[S.at].file} · MEASURED: production/military_lines. The units and time basis of produced/speed/cost are not certified; these fields are not presented as daily output.`);controls.append(rawButton);
  }
  ...
  function render(){
    ...
    $("content").replaceChildren();if(!tags().length)$("content").innerHTML='<div class="empty"><strong>Select at least one country</strong>Use the country filter to restore the seven majors.</div>';else({overview,forces,industry,armor,wars,country}[S.tab])();
