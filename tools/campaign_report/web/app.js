...
async function boot(text) {
  ...
  function equipment(){const el=$("content"),bar=controlBar();segment(bar,[["army","Army"],["tanks","Tanks"],["air","Air"]],S.equipment,v=>{S.equipment=v;S.family="all";});
    ...
    const meta=(key,unit="equipment")=>({unit,evidence:"DERIVED",source:key==="stock"?"production/equipments → variant registry":key==="active_factories"?"production/military_lines":`units/division → equipment/${key}`,note:"Aggregated by equipment-definition family."});
    const g=grid();
    const charts=air?[["stock","Aircraft stockpile","aircraft"],["active_factories","Factories assigned to aircraft","factories"]]:[["stock",`${what}: stockpile`,"equipment"],["deployed",`${what}: in divisions`,"equipment"],["reinforcement_need","Recorded reinforcement requests","equipment"],["active_factories",`Factories assigned to ${what.toLowerCase()}`,"factories"]];
    ...
    if(air){appendMetric(g,"aircraft");const cg=section(el,"Stockpile by role",`At ${dateText(times[S.at])} · signed stock grouped by the airframe role of the checkout's definitions`);tags().forEach(tag=>composition(cg,tag,current(tag)?.air?.stock_types,labelCountry(tag)));}
    ...
    const columns=air?["Country","Variant","Family","Creator","Stock","Factories"]:["Country","Variant","Family","Creator","Stock","Deployed","Factories"];
    const redraw=()=>{const query=search.value.toLowerCase();const filtered=variants.filter(v=>(`${variantName(v)} ${v.name} ${v.definition} ${v.creator} ${v.family}`).toLowerCase().includes(query)&&(origin.value==="all"||(origin.value==="domestic"?v.creator===v.tag:v.creator&&v.creator!==v.tag)));holder.replaceChildren(tablePanel(columns,filtered.map(v=>[countryCell(v.tag),`${esc(variantName(v))}<small>${esc(v.definition || "")}</small>`,esc(labelType(v.family)),esc(v.creator || "—"),fmt(v.stock,1),...(air?[]:[fmt(v.deployed,1)]),fmt(v.active_factories)])));if(!filtered.length)holder.append(notice("No variants in this selection."));};search.oninput=redraw;origin.onchange=redraw;redraw();
