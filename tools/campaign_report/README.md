...
| Stability and war support | **DERIVED** displayed values. The save stores only the base (`countries/TAG/stability`, `war_support`), which `add_stability` / `add_war_support` and weekly modifiers move; national spirits never touch it (**MEASURED**: a country carrying a +0.5 `stability_factor` spirit keeps its stored 0.11). The report adds, per country: the `stability_factor` / `war_support_factor` of the ideas it holds (laws, spirits, ministers defined as ideas), of the traits of its hired advisors and ruling leader (trait names from the save's `character_manager`, values from `common/country_leader`), and of its enabled dynamic modifiers (stored value list, in definition order); the ruling party popularity × 0.15 × (1 + `party_popularity_stability_factor`); −0.2 × (1 − `offensive_war_stability_factor` − `war_stability_factor`) only while waging an offensive war, + `defensive_war_stability_factor` while in a defensive war; coastal protection ratio × 0.1; and, for war support, +0.2 as soon as one war is defensive (else −0.2 when offensive; sides from `war_relation/first_was_instigator`), the `pride_of_the_fleet_country` static modifier while the pride of the fleet is intact (the temporary sunk penalty for 30 days after `pride_of_the_fleet_date_lost`), plus the stored `being_bombed_support_penalty` and `heroes_dying_war_support_penalty`. NCountry constants are vanilla 1.19.2 unless `05_defines.lua` overrides them (it sets the tension impact to 0). **Calibrated** on two in-game readings, August 1941 of campaign 3a1e3a7f: stability ENG 77 % / SOV 100 % (rebuilt 77.6 / 104 → 100), war support SOV 91 % (rebuilt 91.6). ENG war support the same month reads 24 % in game against 34.8 rebuilt: the convoy-raid malus (`CONVOYS_BEING_RAIDED_*`, up to −0.3) and enemy operative propaganda are computed by the engine and not stored in the country block, so a heavily raided country reads too high. **ASSUMED** beyond those readings: the term formulas, the first `country_leader` entry of the ruling party being the leader in office, and the absence of the `war_support_during_war` static modifier (neither scaling of it fits both stability readings); a term that cannot be read leaves the displayed value unknown. Per-term breakdown in `countries/TAG/politics`; the stored bases stay available as `stability_base` / `war_support_base` (**MEASURED**). |
