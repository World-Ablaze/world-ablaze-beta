...
| Industry & resources | Installed civilian/military factories and dockyards; three refinery families with hydro variants and active/inactive detail; production, trade, industry demand and the resulting balance |
...
| Stability and war support | **DERIVED** displayed values. The save stores only the base (`countries/TAG/stability`, `war_support`), which `add_stability` / `add_war_support` and weekly modifiers move; national spirits never touch it (**MEASURED**: a country carrying a +0.5 `stability_factor` spirit keeps its stored 0.11). The report adds, per country: the `stability_factor` / `war_support_factor` of the ideas it holds (laws, spirits, ministers defined as ideas), of the traits of its ruling leader (first `country_leader` entry of the ruling party, leader traits only) and of its appointed advisors (`characters/appointed_advisors`; trait names from the save's `character_manager`, values from `common/country_leader`), and of its enabled dynamic modifiers (stored value list, in definition order). War support then adds −0.2 when the country wages an offensive war and +0.2 when it fights a defensive one (both at once when it has both; sides from `war_relation/first_was_instigator`), the `pride_of_the_fleet_country` static modifier while the pride of the fleet is intact (the temporary sunk penalty for 30 days after `pride_of_the_fleet_date_lost`), and the stored `being_bombed_support_penalty` and `heroes_dying_war_support_penalty`. Stability adds the ruling party popularity × (0.15 + `party_popularity_stability_factor`), the stored `coastal_protection_ratio` as is, an at-war term = −0.2 + `offensive_war_stability_factor` while waging an offensive war + `defensive_war_stability_factor` while fighting a defensive one, and the `war_support_during_war` static modifier (−0.3) × (1 − war support) while at war. The active faction manifest (`faction/goal_status/manifest`, members from `faction/members`) adds its `scale` modifiers × a progress ratio — the Axis conquest manifest gives up to +0.15 `offensive_war_stability_factor`, China's up to +0.1 `war_support_factor`, the Comintern's up to +0.1 `war_stability_factor` — plus its fulfilled modifiers above the threshold. The engine computes that ratio from collections the save does not store; the report rebuilds it (**DERIVED**) from the save's state owners and controllers, the initial cores of `history/states` (in-game core changes are **ASSUMED** rare), the continents of `map/definition.csv` through each state's first province, the ruling parties and the faction leader (**ASSUMED** first member): conquest = faction-controlled non-core states ÷ faction-owned core states (clamped to the manifest's `range max`), China = CHI/PRC-core states controlled by a member sharing the leader's government ÷ all CHI/PRC-core states, Comintern = states on the leader's continent under a communist controller ÷ states on that continent (`countries/TAG/politics/manifest_ratio`). A manifest whose collections are not reproduced is evaluated at both bounds and reported only when they agree after clamping (`manifest_bounds`). NCountry constants are vanilla 1.19.2 unless `05_defines.lua` overrides them (it sets the tension impact to 0). **MEASURED** against the in-game tooltips of August 1941, campaign 3a1e3a7f: ENG stability and war support (71.66 rebuilt / 71.67 listed, 24.77 / 24.78), ITA stability and war support, JAP stability (every listed line reproduced: base, at war, ruling party popularity, spirits, laws, dynamic modifiers, leader, appointed advisors, war-support line, coastal protection, bombing, casualties, pride of the fleet) and the SOV readings (stability 100 %, war support 91 %: 91.6 rebuilt). **ASSUMED**: the leader in office being the first `country_leader` entry, `war_stability_factor` entering the at-war term, manifest `scale` modifiers being linear in the progress ratio, and the absence of espionage propaganda; a term that cannot be read leaves the displayed value unknown. Per-term breakdown in `countries/TAG/politics`; the stored bases stay available as `stability_base` / `war_support_base` (**MEASURED**). |
...
| Generals, field marshals, admirals | **DERIVED** the country's `characters/character_status` entries with `unit_leader=yes`, joined to the `character_manager` records: a `corps_commander` block without `field_marshal` counts as a general, `field_marshal` as a field marshal, `navy_leader` as an admiral. Retired characters sit in `retired_character_status` and are not counted. **ASSUMED**: a leader hidden by an availability trigger is counted like a recruited one. |
| Political power | **MEASURED** `countries/TAG/politics/political_power`, the stored balance at the save date. |
| Aircraft in wings by family | **DERIVED** `strategic_air/TAG/air_wing_pool/air_wings/equipment` amounts per variant (older variants stay listed at 0), mapped through the variant registry to the checkout's airframe families and roles; stored as the `deployed` value of air families and variants and as `air/wings_by_role`. Wings whose variant is not a classified airframe are reported as an issue, never dropped silently. |
| Equipment domains | **DERIVED** from the checkout's `common/units/equipment` types: `armor` for tank chassis, `army` for infantry / artillery / anti-tank / anti-air / motorized / mechanized / rocket / railway-gun equipment, `air` for airframes (fighters, CAS, naval, tactical and strategic bombers, transports). Ships are not classified. |
| Factories assigned to equipment | **DERIVED** sum of `production/military_lines/active_factories` per family. A serialised line without the field is a queued line with no factory yet (the engine omits the field at its default 0) and counts as 0, never as unknown. |
