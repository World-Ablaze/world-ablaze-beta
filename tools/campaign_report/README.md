...
| Stability and war support | **DERIVED** displayed values. The save stores only the base (`countries/TAG/stability`, `war_support`), which `add_stability` / `add_war_support` and weekly modifiers move; national spirits never touch it (**MEASURED**: a country carrying a +0.5 `stability_factor` spirit keeps its stored 0.11). The report adds, per country: the `stability_factor` / `war_support_factor` of the ideas it holds (laws, spirits, ministers defined as ideas), of the traits of hired advisor characters (idea_token → traits → `common/country_leader`), and of its enabled dynamic modifiers (stored value list, in definition order); the ruling party popularity × 0.15 × (1 + `party_popularity_stability_factor`); the war term −0.2 × (1 − offensive/defensive factor − `war_stability_factor`) when at war; coastal protection ratio × 0.1; and, for war support, the offensive (−0.2) or defensive (+0.2) posture read from `war_relation/first_was_instigator`, plus the stored `being_bombed_support_penalty` and `heroes_dying_war_support_penalty`. The NCountry constants are vanilla 1.19.2 unless `05_defines.lua` overrides them (it sets the tension impact to 0). Per-term breakdown in `countries/TAG/politics`. **ASSUMED**: the term formulas, a country in both an offensive and a defensive war counting as offensive, and the absence of espionage propaganda; a term that cannot be read leaves the displayed value unknown. The stored bases are kept as `stability_base` / `war_support_base` (**MEASURED**). |
