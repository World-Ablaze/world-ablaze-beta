...
| **Generators** | `tools/gen/` | When their source changes (map, corridors, armour templates, faction theatres, landing calendar) | `run_generators.py` + `map_generators/` + `core/`, `gen_rail_corridors.py`, `gen_ai_armor_templates.py` + `armor_templates/` (`armor_templates_registry.json` → the medium / modern / heavy ai_template files, their selection ladders and `generated/armor_templates_manifest.json`; replaced `gen_ai_medium_modern_mirror.py` on 2026-09-19), `gen_ai_faction_theaters.py`, `gen_ai_landing_reservations.py`, `gen_ai_research_allow_gates.py` (AI research blocklist → managed `allow` blocks in `common/technologies/`), `gen_air_tech_gates.py` (`air_tech_registry.json` → `WA_AI_PRODUCTION_air_tech.txt`, the AI's air production tech rungs), `gen_grade_pairs.py` (steel-grade / ammunition-grade design twins in `common/ai_equipment/*_tank.txt`, run after the evaluator) |
...
| **Migrations** | `tools/migrations/` | Replayable pipelines, rarely replayed (`ai_will_do/` tech replacers + `ai_replacer_base/`; `prospecting/` decision replacer; `ammo_slot/` writes `ammo_type_slot` into every AI tank design, `--check` is idempotent) | Dry-run first, always |
