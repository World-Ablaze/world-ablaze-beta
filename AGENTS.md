...
| `.claude/skills/wa-constants-registry/SKILL.md` | HOI4 `@` constants are file-scoped: the registry of every constant / engine define / building fact declared in more than one place (`tools/constants_registry.json`), the checker `python tools/check_constants.py` (drift, phantom mirrors, unread copies), owner-vs-mirror rules, per-file renames, and the "a legacy gate needs a recorded purpose" rule. Run the checker before committing any change to `WA_AI_*` effects/triggers, `05_defines.lua`, `00_buildings.txt` costs/caps, or the `savegame.py` `_PC_*` tables. |
...
10. Remember that `@` constants are file-scoped in HOI4 script. If a constant is needed in multiple files, redeclare it where used (with a `# must match <file>` comment) or store it as a global variable during initialization — and register the pair in `tools/constants_registry.json` so `python tools/check_constants.py` holds the contract; a comment alone is not a mechanism (see `.claude/skills/wa-constants-registry/SKILL.md`). Renames are per file. A gate, cap or shared temp you add carries a header sentence naming what it protects and which fact it assumes, or it will outlive its reason.
