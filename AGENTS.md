...
4. Keep generic AI country classification in `WA_AI_CONFIG.txt`. If a rule is based on country archetype, doctrine type, faction role, airforce type, or major/minor status, add or reuse a config trigger instead of copying country tag lists.
