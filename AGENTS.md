...
| `.claude/skills/wa-constants-registry/SKILL.md` | How AI numbers are declared: shared constants are HOI4 1.18 **script constants** in `common/script_constants/wa_ai_*.txt` (`constant:cat.group.key`, one declaration visible from every file); a file-scoped `@` is legal only when one file reads it; cross-format copies (`05_defines.lua`, `00_buildings.txt` costs/caps, `savegame.py` `_PC_*` tables) are registered in `tools/constants_registry.json` and verified by `python tools/check_constants.py` (drift, unresolved `constant:`, unused keys, a `@` shared between two WA files). Also the validated-contexts table (`constant:` does not work in `ai_strategy value =`) and the "a legacy gate needs a recorded purpose" rule. Run the checker before committing any change to `WA_AI_*` effects/triggers, `common/script_constants/`, `05_defines.lua`, `00_buildings.txt`, or `savegame.py`. |
