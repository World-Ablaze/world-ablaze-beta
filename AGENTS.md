...
`WORK.md` (repo root) is the single tracker: one **SUBJECT** = one intended behaviour, named by a
stable slug (`lend-lease-relief`, `na-corridor`). Commits, code comments, console harness and
campaign probe all attach to the slug. It replaced QUEUE.md, the per-fix checklist items and the
fix-number system on 2026-08-23 (`documentation/PROCESS_REDESIGN_PROPOSAL.md`;
`documentation/FIX_HISTORY.md` resolves historical `Fix NN` numbers).

- **One session = one subject.** At most **4** subjects in OPEN; the rest are PARKED with their
  state. A discovery outside the session's subject does not get fixed on the way past — it becomes
  at most a *proposed* line, and the admission rule decides.
- **Admission**: a subject enters WORK.md only if the owner asked for it, or it rests on a
  MEASURED symptom (save, playthrough report) about MOD behaviour. Meta-work (tooling, docs,
  checker, skills) never enters without an explicit owner request. Default = do not add; mention
  the observation in one line at end of session.
- **Big scripted-effect changes require an owner-run console test.** A commit touching a
  `WA_AI_*` scripted effect/trigger of a system with a `WA_TEST_*` harness puts its subject in
  `SHIPPED-UNTESTED`; only the owner running the harness in the game console (output pasted into
  WORK.md) moves it to `TESTED`. A big change (> 40 lines, or any signature/scope change of an
  effect called by an on_action) to a system *without* a harness includes writing one. This rule
  exists because lend-lease shipped untested and ran broken for a week with no signal.
- A subject without a closing criterion is a wish, not a task. `python tools/check_worklist.py`
  enforces WIP limit, stale `SHIPPED-UNTESTED`, missing criteria, harness contract and the BOM
  rule — its docstring is the complete list.
...
**2. Country tags live in `WA_AI_CONFIG.txt` — nowhere else.** All country classification goes through archetype triggers in `common/scripted_triggers/WA_AI_CONFIG.txt`. Before writing `tag =` / `original_tag =` anywhere else, reformulate the rule as an archetype question ("is this a major Axis land power?") and add or reuse a CONFIG trigger. **A tag is never shorthand for a TECH TREE.** "GER has the German armour tree" is not a classification anyone gets to hand-copy: it is the `available` block of `ger_armour_folder`, and the answer is `WA_AI_TECHTREE_has_ger_armour_folder` — tag OR `german_technologies_tree_flag`, so an adopted tree counts. If a CONFIG list exists only because those countries' trees carry a branch (the old `use_light_tank_destroyers = { ITA JAP GER }`), it belongs in `WA_AI_TECHTREE_has_branch_*`, not here; what stays in CONFIG is the doctrine switch on top of it. The Country layer (`WA_AI_MILITARY_COUNTRY_<TAG>*`, `events/WA_AI_<TAG>.txt`, `common/ai_equipment/<TAG>_*`) is the sole sanctioned exception, reserved for behaviour genuinely unique to one nation — treat every addition there as a design decision to justify, not a convenience, and ask first whether an archetype trigger would cover it.

**3. Change existing systems only with an impact analysis.** Existing behaviour is load-bearing, encodes fixes for cases that already broke (`# [slug]` markers; historical `# Fix NN:` comments resolve via `documentation/FIX_HISTORY.md`), and misbehaves silently — regressions surface mid-campaign, not at parse time. Before modifying an existing trigger, effect, or strategy block: (a) enumerate every caller and reader (grep the name across `common/` and `events/`); (b) identify which countries, archetypes, and cadences reach it; (c) walk both the historical and an ahistorical scenario through the change; (d) check `wa-lessons-learned` and surrounding `# Fix NN:` comments for the case the current code encodes — send the decision to the `wa-lessons-reviewer` subagent (`.claude/agents/wa-lessons-reviewer.md`) rather than reading the 1800-line log inline, and — in parallel — to the `wa-architecture-reviewer` subagent (`.claude/agents/wa-architecture-reviewer.md`), which checks the change against the constants registry, the PC allocation model, the WA_TLM honesty rules and the editing rules below; treat a CONFLICT verdict from either as blocking; (e) state the regression risk explicitly in your summary; (f) if you claim a residual is "bounded", "self-healing" or "at most N", show the t0/t1/t2 table at the real cadences (pulse interval vs completion time, monthly save vs 2-day event) — the adjective without the table is how Fix 81 cut 1 shipped a stall it called bounded; (g) if a reporter or colleague proposed a fix and you are choosing another, quote their objection and write the sentence "mine covers it because …" — if it cannot be written, keep theirs. Prefer additive, gated changes over in-place rewrites of shared code paths. When the risk is unclear, diagnose first — the first hypothesis about AI misbehaviour in this codebase is usually wrong.

**4. The layer is carried by the name.** WA_AI code is organised in four technical layers —
DECLARATION (`WA_AI_CONFIG*`, `common/script_constants/wa_ai_*`), OBSERVATION (`WA_AI_<SYS>_is_*`
/ `_has_*`), DECISION (`WA_AI_<SYS>_should_*` / `_can_*`), CONSUMPTION (`common/ai_strategy/`,
`common/scripted_effects/`, `events/`) — and the name prefix/verb is what makes the layer
checkable by grep. The three frontier tests, the named-exception lists and the full model live in
`documentation/WA_AI_LAYERS.md`; `python tools/check_ai_layers.py` enforces them mechanically
(ratchet: existing debt is frozen in `tools/ai_layers_baseline.json`, adding new debt fails).
These technical layers are ORTHOGONAL to the military SCOPE tiers (Default/Region/Faction/Country
of `WA_AI_MILITARY_SYSTEM.md`): say "layer" for the technical axis, "tier"/"échelon" for scope.
...
| AI configuration and country archetypes | `common/scripted_triggers/WA_AI_CONFIG.txt` | THE single point of entry for AI configuration (owner ruling 2026-08-30: one file, never split): difficulty, major/minor classification, doctrine/airforce archetypes, the shared CALENDAR WINDOWS section (named date bounds) and the ALWAYS-FLAGS section (yes/no switches). The only WA_AI home for country tags. Prefer adding reusable config triggers here over scattering `tag =` or `original_tag =` checks. **A trigger whose body reads only OTHER triggers is not configuration** and does not live here: it is a layer-2 verdict and belongs to the system that owns the question — `common/scripted_triggers/WA_AI_COUNTRY_triggers.txt` (`WA_AI_COUNTRY_is_minor`), `WA_AI_DIFFICULTY_triggers.txt` (the verdicts composed from the five difficulty levels; the raw `difficulty` comparisons stay in CONFIG), `WA_AI_NAVY_triggers.txt`, `WA_AI_DOCTRINES_land.txt`, `WA_AI_TEMPLATES_triggers.txt`, `WA_AI_MILITARY_triggers.txt`. CONFIG itself must never read one back — that is a layer inversion. Layer-3 gate triggers (one per ai_strategy gate) live per system: `WA_AI_NAVAL_triggers.txt`, `WA_AI_MILITARY_<DOMAIN>_gate_triggers.txt`, `WA_AI_PRODUCTION_*_triggers.txt` — see `documentation/WA_AI_LAYERS.md`. |
...
| Map data, pathfinding, and math helpers | `common/scripted_effects/WA_AI_MAP_effects.txt`, `common/scripted_effects/WA_AI_MAP_province_connections.txt`, `common/scripted_effects/WA_AI_MAP_province_railway_connections.txt`, `common/scripted_effects/WA_AI_MAP_state_provinces.txt`, `common/scripted_effects/WA_AI_MAP_state_vp_provinces.txt`, `common/scripted_effects/WA_AI_MAP_province_coordinates.txt`, `common/scripted_effects/WA_AI_MAP_province_terrain.txt`, `common/scripted_effects/WA_AI_MAP_landmass_data.txt`, `common/scripted_effects/WA_AI_pathfinding_effects.txt`, `common/scripted_effects/WA_AI_MATH_effects.txt`, `tools/gen/map_generators/`, `tools/gen/run_generators.py` | Generated map lookup data supports province pathfinding, railway logic, landmass detection, state mappings, and distance calculation. Do not hand-edit generated `WA_AI_MAP_*` data files unless there is no viable generator path. |
| Technology trees (who owns which tree, what a tree contains) | `common/technology_tags/00_technology.txt` (SOURCE), `common/scripted_triggers/WA_AI_TECHTREE_membership.txt` + `common/scripted_triggers/WA_AI_TECHTREE_gates.txt` (both GENERATED), `tools/techtree_registry.json`, `tools/gen/gen_techtree_membership.py`, `tools/gen/gen_techtree_gates.py` | **A country tag never stands in for a tech tree.** Membership (`WA_AI_TECHTREE_has_<folder>`) is copied straight from each folder's `available` block — tag OR `<x>_technologies_tree_flag`, so a country that ADOPTED another tree (`common/decisions/_unique_technologies_adoption.txt`) is answered on the tree it owns. Two questions, never merged: `WA_AI_TECHTREE_has_branch_<cap>` = does my TREE carry this branch (the answer that replaces the old tag lists); `WA_AI_TECHTREE_has_<cap>` = do I hold a rung now. `has_` deliberately carries **no** membership term — 40 sites grant a national-tree rung to a non-member without the flag (`national_focus/finland.txt` alone grants from seven foreign trees), so ANDing membership into availability would delete their templates. The air family has the same pair under `WA_AI_PRODUCTION_has_branch_<line>` (`tools/air_tech_registry.json` key `tree_folders`). |
| AI research weighting | `common/scripted_triggers/WA_AI_RESEARCH_*.txt`, `common/scripted_effects/WA_AI_RESEARCH_effects.txt`, `common/technologies/*.txt`, `tools/migrations/ai_will_do/ai_will_do_replacer_all.py`, `tools/migrations/ai_will_do/ai_replacer_base/`, `tools/migrations/ai_will_do/REFACTORING_SUMMARY.md` | Research triggers drive `ai_will_do` blocks in technology files. Shared parser/generator code lives in `tools/migrations/ai_will_do/ai_replacer_base/`. Preserve existing trigger logic when regenerating `ai_will_do`. A variant `needs_*` trigger asks three questions, one per layer: fields armour at all / doctrine switch (`WA_AI_CONFIG_DIVISIONS_use_*`) / tree carries the branch (`WA_AI_TECHTREE_has_branch_*`). |
| Resource needs and prospecting | `common/scripted_triggers/WA_AI_RESOURCE_NEEDS_triggers.txt`, `common/decisions/_resource_prospecting.txt`, `tools/migrations/prospecting/needs_aware_generator.py`, `tools/migrations/prospecting/prospecting_decision_analyzer.py`, `tools/migrations/prospecting/ai_will_do_replacer_prospecting.py`, `PRESERVED_MODIFIER_FIX_FINAL.md` | Prospecting AI uses reactive, cooperative, and proactive layers. Preserve country-specific modifiers and indentation when regenerating decision `ai_will_do` blocks. |
| AI production and equipment behavior | `common/ai_strategy/WA_AI_PRODUCTION_*.txt` (air lines: `WA_AI_PRODUCTION_DEFAULT_air.txt`, enabled by the `WA_AI_PRODUCTION_should_open_<line>_line` decisions of `WA_AI_PRODUCTION_air.txt`), `common/ai_strategy/World_Ablaze_production_air_strategies.txt` (air factory share, tag+date blocks), `common/scripted_triggers/WA_AI_PRODUCTION_*.txt` (`WA_AI_PRODUCTION_air_tech.txt` is GENERATED from `tools/air_tech_registry.json`), `common/scripted_effects/WA_production_strategy_effects.txt` (the air pulse keeps the purge books only), `common/scripted_effects/WA_AI_PRODUCTION_air_budget.txt` (the monthly land-air `unit_ratio` reconcile), `documentation/WA_AI_AIR_PRODUCTION.md`, `common/ai_equipment/*.txt`, `common/decisions/z_WA_ai*.txt` | Handles production defaults, air production lines, lend-lease production, equipment designs, and purge/fix decisions. Keep general production rules in shared WA_AI files and country-specific tuning in country-specific strategy/equipment files. `common/ai_equipment/` keys follow `documentation/AI_EQUIPMENT_NAMING.md` (design key = the chassis/airframe/hull `type` it targets; `python tools/check_ai_equipment_names.py` enforces it). |
...
| Military theatre and campaign features (landing freeze, Italian theatre, East Africa, Ethiopian war, Tunis bridge, faction theatres, Commonwealth handoff, Afrika Korps window, Allied/Axis air policy, Mediterranean convoy interdiction, Mediterranean Fleet, scripted-target reservation, Allied total commitment) | `documentation/WA_AI_MILITARY_SYSTEM.md` §10–§23 — each section names its files, its triggers and its checklist probe | One section per feature lives in the doc, not here. Shared invariants: **all behavioural switches live in the scripted-trigger "control panel" sections (`WA_AI_MILITARY_triggers.txt`, `WA_AI_LANDING_triggers.txt`) — change behaviour there, never by editing the strategy blocks**; everything keys on dynamic geography (owner / controller / faction / war), never on a tag, a date or an event flag. Faction theatres (§15) are GENERATED — change `tools/gen/gen_ai_faction_theaters.py`, never the file. |
...
4. Respect the layer model (design principle 4): tags, dates and thresholds are DECLARATION-layer data. Country classification goes in `WA_AI_CONFIG.txt` archetype triggers; a shared date lives in a named CONFIG trigger (NEVER in a script constant — `date > constant:` is silently always-true, MEASURED 2026-08-29); a shared number lives in `common/script_constants/`; and a `common/ai_strategy` `allowed`/`enable` block names decision triggers (`_should_`/`_can_`) instead of re-implementing their terms. Raw `difficulty` comparisons are forbidden outside `WA_AI_CONFIG.txt` — the difficulty mapping is non-monotonic (`[difficulty-mapping]`), so `difficulty > N` cannot express "normal or harder"; gate on the named `WA_AI_DIFFICULTY_*` triggers. For `common/ai_strategy/WA_AI_MILITARY_*` specifically, `tag = X` and `original_tag = X` remain forbidden as **gating** terms outside Country-layer files (see `documentation/WA_AI_MILITARY_SYSTEM.md`); single-tag literals (e.g. `NOT = { tag = ITA }`) and Country-file addressing (`allowed = { tag = GER }`) are permitted, and tag references inside `ai_strategy = {}` payload (`id = "USA"`, `target = ...`) are payload, not gating. `python tools/check_ai_layers.py` enforces all of this as a ratchet — pre-existing debt is frozen, new debt fails.
...
7. Add clear scripted triggers and effects instead of inline complex blocks. Good triggers answer one positive question, for example `WA_AI_should_prospect_resource_steel`. Comments follow one rule: **one comment per protected behaviour, named by its subject, no history** — `# [slug] what this protects; the fact it assumes; how to tell that fact is gone`, plus the architecture decisions and rejected alternatives a reader needs to touch the code safely. An effect/trigger header is at most 5 lines. Forbidden at the code site: `# Fix NN` (historical numbers resolve via `documentation/FIX_HISTORY.md`), fix-on-fix chains, campaign measurements, dates, reviewer exchanges — git and `WORK.md` carry the history; durable rules go to `wa-lessons-learned`. A 55-line header over a six-line trigger is a defect, not thoroughness. Finally, comments drift while code does not: verify any cadence, threshold or call site a comment states against the source before you rely on it — three separate comments called `WA_AI_needs_<r>` a *monthly* counter when its only writer runs on a ~2-day event.
...
| `common/scripted_triggers/WA_AI_PRODUCTION_air_tech.txt` (the AI's `has_worthwhile_*` air tech gates and their `has_branch_*` capability twins) | `tools/gen/gen_air_tech_gates.py` from `tools/air_tech_registry.json` (`--check` / `--dry-run`); the registry's `exclude` list is also read by `tools/equipment_evaluator`, its `tree_folders` key binds each tree to its technology folder |
| `common/scripted_triggers/WA_AI_TECHTREE_membership.txt` (who owns which technology tree) | `tools/gen/gen_techtree_membership.py` from `common/technology_tags/00_technology.txt` (`--check` / `--dry-run`) |
| `common/scripted_triggers/WA_AI_TECHTREE_gates.txt` (the `has_branch_*` / `has_*` capability gates) | `tools/gen/gen_techtree_gates.py` from `tools/techtree_registry.json` (`--check` / `--dry-run`) |
...
```powershell
python tools/gen/run_generators.py all --dry-run
python tools/gen/run_generators.py all
```

`tools/README.md` indexes the folder by usage tier: gates at the root (`check_*`, harness/log readers), `gen/` (regenerate when the source changes), `migrations/` (replayable pipelines, rarely replayed), `archive/` (one-shot, already applied, unmaintained).

Run AI `ai_will_do` tooling in dry-run mode before applying changes, following `tools/migrations/ai_will_do/REFACTORING_SUMMARY.md` for current parser status and limitations.
...
| --- | --- |
| Map/pathfinding/railway generation | Run the relevant `tools/gen/run_generators.py` generator with `--dry-run`, then run without `--dry-run` only when output changes are intended. Review `documentation/WA_AI_RAILWAY_SYSTEM.md` and related test docs. |
...
| Any change to a `common/ai_strategy/WA_AI_*` gate block, a `WA_AI_CONFIG*` trigger, a literal date/threshold in a `WA_AI_*` file, or a `difficulty` comparison | Run `python tools/check_ai_layers.py` (exit 0 required; `--update-baseline` in the same commit when a ratchet count drops). `tools/ai_layers_baseline.json` is generated — never hand-edit. `--selftest` after any change to the checker itself. |
| Any change to `WORK.md`, `references/checklist.md`, or a `WA_TLM_*` write site | Run `python tools/check_worklist.py` (exit 0 required). It is also the pre-flight before scoring a campaign. A subject in `SHIPPED-UNTESTED` needs the owner's console-harness run — pasted output — to move on. |
...
| Any new or modified `WA_TEST_*` console harness (a scripted effect that logs measurements) | Follow the **harness contract v1** in `wa-testing`: context header printed first (who/scope lines + known-false control + STOP rule, inline, never behind a shared helper), independent walk, own event file — never `events/wa_events_test.txt` (its call site poisoned country-valued triggers, cause unknown). `python tools/check_worklist.py` enforces the header mechanically (`HARNESS-CONTRACT`). |
| Any edit to `tools/air_tech_registry.json` or to the AI's air tech gates | Run `python tools/gen/gen_air_tech_gates.py --check` (exit 0 required); regenerate `common/scripted_triggers/WA_AI_PRODUCTION_air_tech.txt` with the generator, never by hand. A `TREE-GAP` WARN is a tree deliberately barred from a line - confirm it is a decision. |
| Any edit to `common/technology_tags/00_technology.txt`, `tools/techtree_registry.json`, or a `WA_AI_TECHTREE_*` reader | Run `python tools/gen/gen_techtree_membership.py --check` then `python tools/gen/gen_techtree_gates.py --check` (both exit 0). Both output files are GENERATED - never hand-edit. A **`BRANCH-GAP` ERROR (exit 2)** names a folder that carries the branch in `common/technologies/` but is in neither `trees` nor `branch_extra` - it re-derives the candidates from the tech files, so the registry cannot vouch for itself; resolve by adding the rungs or naming the folder in `branch_extra` with its reason. A `TREE-GAP` WARN is the weaker sibling-capability check. Before adding a membership term to an availability (`has_*`) gate, re-read why it has none: a rung granted by focus or event reaches countries outside the folder. |
| Any edit to `common/ai_equipment/` | Run `python tools/check_ai_equipment_names.py` (exit 0 required). Group keys are `<OWNER>_<role_slug>`, design keys are the exact `target_variant.type` (plus `__<qualifier>` only for a second design on the same type), every design line carries its `# <display name>`; the convention and its rationale are in `documentation/AI_EQUIPMENT_NAMING.md`. `plan` / `apply` migrate; the tool rewrites the `WA_EQUIPGEN` marker ids with the keys. |
