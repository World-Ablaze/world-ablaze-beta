...
| `common/scripted_triggers/WA_AI_PRODUCTION_air_tech.txt` (the AI's `has_worthwhile_*` air tech gates) | `tools/gen/gen_air_tech_gates.py` from `tools/air_tech_registry.json` (`--check` / `--dry-run`); the registry's `exclude` list is also read by `tools/equipment_evaluator` |
