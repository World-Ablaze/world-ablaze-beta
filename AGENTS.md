...
| Military theatre and campaign features (landing freeze, Italian theatre, East Africa, Ethiopian war, Tunis bridge, faction theatres, Commonwealth handoff, Afrika Korps window, Allied/Axis air policy) | `documentation/WA_AI_MILITARY_SYSTEM.md` §10–§18 — each section names its files, its triggers and its checklist probe | One section per feature lives in the doc, not here. Shared invariants: **all behavioural switches live in the scripted-trigger "control panel" sections (`WA_AI_MILITARY_triggers.txt`, `WA_AI_LANDING_triggers.txt`) — change behaviour there, never by editing the strategy blocks**; everything keys on dynamic geography (owner / controller / faction / war), never on a tag, a date or an event flag. Faction theatres (§15) are GENERATED — change `tools/gen_ai_faction_theaters.py`, never the file. |
