...
- **History:**
  ...
  - 2026-08-11 · `31eaf7e6` · FAILED — from-start scoring: USA 16.1% overseas at 1943.6 (bar ≥25%), i.e. 83.9% CONUS — a REGRESSION vs the 62% pre-fix baseline; 24.6% at 1944.6, still under the bar. Only ~700 planes across the Pacific islands at 1944.6; zero UK basing in 1943. Overseas placement is garrison scatter (100–300-plane singletons at Jamaica, Bahamas, Bermuda, Baja California, Lisbon, across four continents), not theatre mass. Root-cause candidate corroborated by R8/R16 reads: USA adds only +13 net air-base levels globally in three years — there is nowhere in-range to forward-base, so the pulls have nothing to bite on. Fix the capacity side before touching the pull values.
