...
| C2 | `front_control` | **CONTRADICTION** | "Per area, last-set wins per mode"; Phase 5 will add scripted mutual-exclusion triggers | `priority = 0  # Default 0, higher prio strats will override lower` | WA-B:90, WA-B:156-166 vs ENGINE:292 |
...
| X1 | `front_control priority` | **UNUSED CAPABILITY** | Phase 5 will build scripted `*_owns_*` mutual-exclusion triggers to enforce Country > Faction > Default | the engine has a native integer precedence field | WA-B:156-166 vs ENGINE:292 |
