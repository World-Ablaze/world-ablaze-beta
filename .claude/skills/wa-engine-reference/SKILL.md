...
| Which `ai_strategy` blocks are ARMED for a country right now, and at what weighted value? | **the running game** - owner types `tag <TAG>` then `imgui show ai-strategy` in the console. Native engine window (`Active strategies` and `Weighted Value` each occur once in `hoi4.exe`, control string zero). It is the only source that maps a live entry back to its script NAME - the savegame stores unnamed numeric type codes. Owner-run only, one country, no history; the `Target` column decode is **not established**. Full contract and the three misreads: skill `wa-diagnosis`, technique 5 |
| Is the engine actually launching an AIFC concentrated push, on which front, with how many units? | **the running game** - `tag <TAG>` then `imgui show ai_force_concentration`. Per-front `AIFC score` and the active plan's `Should receive` / `assigned units` / `strategic Targets` are engine-runtime, never serialised; the save carries only the push's existence + target + freshness (`aifc.py` `active push`). Full field-by-field split: skill `wa-diagnosis`, AIFC window section |
