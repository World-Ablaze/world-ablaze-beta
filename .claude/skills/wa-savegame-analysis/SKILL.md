...
**Measuring is rung 2 of six.** If the task asks *why* rather than *what*, this skill only supplies the measurement and the mod state; the answer is not finished until it names a script line or a documented engine boundary. The output contract for that is skill `wa-diagnosis` - read it before writing the conclusion, not after.
...
| Command | Purpose |
| --- | --- |
| `list [--dir D] [--limit N]` | Saves newest-first with date, player, campaign id, session, size. `--limit 0` = unlimited. |
| `campaigns [--dir D]` | Group all saves by campaign identity, date-ordered within each group. **Start here when selecting saves.** |
| `meta FILE` | One save's header: player, date, version, session, seed, unique id, mods, ironman, start_date. In `version=… v1.19.3.0.c01a (5122)` the MOD checksum is the parenthesised field (it changes with any loaded mod file — the proof an edited file was loaded); `c01a` is the engine build. |
...
| `tlm TAG FILE... [--match RE]` | WA_TLM telemetry dashboard: scalars (stamps decoded to dates) + ring-buffer series paired with the `wa_tlm_hist_t` axis. **Every value row carries its own freshness**: a family whose `_last_t` lags the save by more than 2 months is annotated `FROZEN Nmo (last sampled YYYY.M)` on the row itself, and `_last_t = 0` reads `[never sampled]`. **Only a `_hist` array gets a date axis**; any other indexed array is printed as a `table` with raw indices (`[13] 450`), never as a time series. Contract and metric registry: `documentation/WA_TLM_TELEMETRY_SYSTEM.md`. |
| `navy TAG FILE... [--fleets]` | Fleet / task-force / **mission** breakdown: ships by mission, escort-hull (`screen_ship`) counts, idle share, and how much of the idle sits in fleets with no admiral. The mission enum is named nowhere in the game files — the map lives in `_NAVAL_MISSIONS` in the script (checklist R36). `--fleets` also prints, per fleet, a **`heavy=` column** (battleship / battlecruiser / heavy cruiser / carrier, substring-matched against `_NAVAL_HEAVY` so WA's per-country hull names still count) and the **list of strategic-region ids** the fleet operates in — `-` means none, which for a fleet with no admiral means parked in port. Added 2026-08-21 for checklist R97: "are the capital ships in the Mediterranean" cannot be answered from a region *count*. |
...
| `resources TAG FILE...` | Per-resource ledger: `produced`, `transfer_overlord_subject`, `imported`, net available supply (`to_use[0]`), industry demand (`to_use[2]`), `to_export`, actually `exported`, plus the balance identity's residual. |
| `buildings TAG FILE... [--match RE]` | Building levels summed over states, **owned vs controlled** columns, with each `*_inactive` twin listed under its active counterpart. This is how refinery on/off becomes visible (R29). **A `--match` that can only hit a province-scoped building (`naval_base`, `rail_way`, `supply_node`, `naval_supply_hub`, bunkers) is refused with an error and a non-zero exit** — the tool now enforces the gotcha below instead of returning an empty table. |
| `decisions TAG FILE... [--match RE]` | Decodes `decision_status`, labelling each entry kind separately: live `active_*`/`decision_to_*` entries carry a `days` countdown; `random_item` entries carry a **cumulative per-(decision, target) counter whose ratio to real firings is not derivable from a save** — the command prints per-decision TOTALS across targets, marks `fire_only_once` rows, and calibrates the counter on the save in front of you. |
| `pc TAG FILE... [--match RE] [--limit N]` | Priority-construction queue: every project with its building type, strategy tag and priority band **by name**, plus a per-country summary (factory share, allocation override, gate counters) and the queue-invariant alarms. Absent and 0 are printed apart — see the gotcha below. `--match` filters the composed `<building> <strategy tag> <priority band>` **label only** — never the WA strategy name or a `# Fix nn` word — and now says so on a zero match. `--limit 0` = unlimited. |
...
**Companion: [stock.py](scripts/stock.py) — who BUILT the equipment a country is holding.** `stock.py TAG[,TAG...] FILE... [--match RE] [--creator TAG] [--all] [--foreign] [--limit N]`. The save's top-level `equipments={}` block defines one variant per (equipment name, creator country), so a holder's stockpile entry against a variant whose `creator` is not the holder is equipment it did not build. **This is the only way to read the RECEIVING end of a transfer** — the `WA_TLM_llr_*` family counts what the donor sent, and a send that silently moves nothing looks identical to one that works (exactly how `send_equipment` passed review while carrying no convoys). It keeps the two delivery paths apart, which is the whole point: the **stockpile** is what `add_equipment_to_stockpile` writes and the equipment is OWNED, while **`foreign_lease_equipments`** is vanilla lend-lease, a loan that returns. Two gotchas: `--match` filters the **variant** name as stored, not the archetype — WA's German rifles are `ger_inf_*` / `ger_hv_inf_*`, so `--match infantry` returns nothing and `--match "_inf"` returns them — and **amounts are a stock, not a flow**, so consumption and losses are already folded in; read against `wa_tlm_llr_sent_amount` as magnitude, never as equality. A negative amount is a real engine state (committed beyond what is on hand), left signed rather than clamped. One streaming pass, several tags at once — never call it once per tag over the same handle. **`--all` is NOT the free stockpile**: it sums every `equipment={ id amount }` entry in the country block, so division-held and training-queue equipment is added in (MEASURED 2026-09-21, GER `d6190fb7` 1943.9: `--all` reports 152 modern chassis while the `production > equipments` stockpile block holds 0). For "how many are free in stock" read that block directly — `tools/archive/modern_switch_experiment/wa_armor_lib.py` does.

**Companion: [seashare.py](scripts/seashare.py) — replicate an AI gauge out of the save instead of waiting for a campaign.** `seashare.py FILE TAG[,TAG...] [cap|-] [war|faction]` recomputes `WA_AI_PRODUCTION_coalition_sea_share` (Fix 115/116/117): the share of a coalition's industry its leader cannot reach overland. Written to pre-test checklist R79 leg 1 — *does the gauge separate a land coalition from a maritime one* — before shipping the calibration, and it earned its keep: with the repo's usual three-term "our side" it put Germany at **0.254** against a 0.25 bar, because Japan shares the Axis's enemies without sharing its faction. Restricted to the faction, 0.056. **The general lesson is the reusable part:** a scripted gauge whose inputs are all in the save can be replayed in Python and calibrated against real campaigns first. Three traps it documents, each of which produced a confident wrong table: `capital=` is a STATE id, not a province id; a state omits `controller=` when it equals its owner, so reading the field literally leaves the map uncontrolled; and its BFS is PESSIMISTIC against the engine's best-first A*, so run it uncapped for the honest connectivity reading.

**Every `--match` in the table prints what it caught.** A selecting pattern prints `MATCHED n of N <things>: <the distinct names>`; a pattern that selects nothing prints `NO MATCH: pattern 'p' matched 0 of N candidate <things>` plus the available vocabulary. Exit code stays 0 for a legitimately empty result — the line is the signal. Same for `regions.py --grep`.

**Companion: [lendlease.py](scripts/lendlease.py) — the whole lend-lease picture per save: who gives, who receives, over WHICH channel.** `lendlease.py FILE... [--tag TAG] [--min-ic N] [--limit N]`. Three tables it never conflates: **VANILLA** (engine, maritime) from the per-pair `lend_lease_to_allies_history` cumulative IC/fuel ledger (giver-side rows, recipient-side mirror reconciled and mismatches flagged, live `lend_lease={first=}` relations with start dates, receiver `recently_leased_ic`); **SCRIPTED relief donor side** (`wa_tlm_llr_sent_n/_amount^idx`, archetype-labelled, rows 1–9 overland + 10 maritime convoy); **SCRIPTED recipient pair matrix** (`wa_tlm_llr_recv_donor/_n/_amount`, builds v32+ — donor ids scope-decoded; on a pre-v32 save it says the donor→recipient split is not recoverable instead of guessing) plus the recipient/donor counters and the decoded `wa_ai_lend_lease_targets`. Passing several saves of one campaign date-orders them and prints per-pair **`d/save` deltas — the cumulative counters are how monthly saves become flows**. What no save carries: the vanilla channel's per-equipment split (engine black box) and convoy losses en route; the receiving stockpile side is `stock.py` (a stock, not a flow — see its gotcha above).
...
- `HOI4bin` — ironman/binary. **Not parseable**; tell the user that save can't be analyzed (they can re-save non-ironman).
  A NON-ironman local game also writes `HOI4bin` while the engine's own `settings.txt` (in the game's user folder, next to the save dir - not a repo file) carries `save_as_binary=yes` — the default; the cloud test build writes text. Fix: game CLOSED (the exe rewrites that file on exit, and takes ~2 min to really exit), set `save_as_binary=no`, then load each binary save and re-save it while paused — no need to replay the run (MEASURED 2026-09-21, `test7a-c`).
...
- **`resource@X` in script reads `to_use[0] + to_use[2]` — available supply MINUS what the industry asks for.** Neither `produced` nor `net` alone. The `resources` command prints it as the **EFFECTIVE** column, and it is the number the game's own top bar shows: green when positive, red when the industry asks for more than there is. ENG aluminium at 1942.6 is net `807.3`, demand `−799.0`, **effective `+8.3`** — which is why its `> 50` latch stayed shut while the net column looked like 16× headroom. Measured 2026-08-13 on campaign `02bd4445` against the `WA_AI_EQUIPMENT_*` latches: **36 discriminating (country, date, resource, bar) cells all side with net+demand and zero with net**, with pass/fail bracketing the bars to within one unit. *The superseded "net alone" rule was drawn from ENG **bauxite** in the same save, whose demand happens to be −1.0 so both readings coincide there — one resource where the two agree is not evidence for the general rule.* The `produced`-alone mistake is still a mistake (it nearly produced a false FAIL on R25); this correction makes the *right* column two columns over.
  - **`to_use[2]` is INDUSTRY DEMAND, not demand left unserved — do not call it a deficit.** The unserved part is `effective` when it is negative; a positive `effective` is a surplus and nothing is unmet. Measured 2026-09-20 on campaign `84ae4038`: at 1942.9.1 as USA the save carries steel `to_use[2] = −1204` while the in-game bar reads a green **+1522** (= `effective`), and across the seven majors and 155 saves **64% of observations with a positive `effective` still carry a `to_use[2]` below −1**. Positive control: `−to_use[2]` correlates **0.89–0.99** with a country's factory count and **~0.00** with its actual shortfall, so the column tracks the size of the industry, not any scarcity. The campaign report charted it as "Unmet demand" for exactly this reason and read as though every major spent the war starving.
- **`decision_status` mixes two entry kinds whose numbers mean different things.** `active_timed_decision={ decision= days= state= }` exists **only while live**: with `state=active`, `days` is time *remaining* (≤ the decision's `days_mission_timeout`); with `state=failed` or `re_enable_cooldown` the same field is a **re-arm cooldown on a different clock** — verified unrelated to the timeout (`economy_fatigue_export_focus_mission` has `days_mission_timeout = 70` and shows `days=13` when failed; `iron_shortage_ai` has timeout 9 and shows 12). `random_item={ decision= count= target= }` is not a state and not days at all — `count` is a **monotone cumulative counter kept per (decision, TARGET) pair**. Conflating the two kinds has already happened twice in one session; use the `decisions` command, which labels them apart.
  - **`count` is NOT a verified firing count, and the "it reads twice the real firings" rule of thumb is FALSE.** That remembered multiplier was applied to a `coal_prospecting` probe and halved a real reading (2026-08-17). The tool now calibrates the counter on the save in front of you: it marks every row whose decision is `fire_only_once = yes` (so it can fire at most **once per target**) and prints the spread of their counts. On GER `9d83084c` 1946.2 those rows read **1, 2 and 4** — three different values for one firing — so no single ratio exists in either direction. Read `count` as monotone *activity*, comparable across saves for the same (decision, target); for an exact firing count use a scripted counter or a WA_TLM metric.
  - **It is per target, not per decision.** A targeted decision has one `random_item` row per target — GER's `coal_prospecting` has 11 rows summing to 64 — so a decision total is the sum, never one row. `decisions` prints the TOTAL line for every multi-target decision.
...
- **A `--match` that matched nothing, and a `--match` that matched too much, both now say so — do not read past those lines.** Every `--match` in `pc`, `buildings`, `tlm`, `ideas`, `decisions`, `flags` (and `regions.py --grep`) prints either `MATCHED n of N <things>: <the distinct names>` or `NO MATCH: pattern 'p' matched 0 of N candidate <things>` followed by the available vocabulary. Exit code stays 0 for a legitimately empty result — the line is the signal, not the status. Two real defects this closes: `pc TAG --match corridor` was used for two checklist items and could never match, because `pc --match` filters the composed `<building> <strategy tag> <priority band>` label (`railway rail rail-war`) and never the WA strategy's own wording; and `--match Somaliland` silently swept French *and* British Somaliland into one country's totals.
- **`--limit 0` means unlimited everywhere it appears** — `savegame.py list/pc/control`, `section --max-lines 0`, `plans.py --limit 0`, `airload.py --top 0`. It used to truncate to nothing on some of them, and a scoring probe deleted its own evidence that way.
...
- **`naval_base` is not in the state blocks of these saves** — every state's `naval_base` sums to 0 in `1944.6_Jun` (`a232d96c`); naval bases are province-scoped in the top-level `provinces={}` block, along with `rail_way`, `supply_node`, `naval_supply_hub`, `bunker` and `coastal_bunker`. `buildings … --match naval_base` used to read a silent 0 for every country — it only ever sees the state block. **The tool now refuses it**: a `--match` that can only hit a province-scoped building (derived from `province_max` in `common/buildings/00_buildings.txt`) exits non-zero naming the key and pointing at `control SCOPE FILE --buildings`, which sums them by the province's real holder (Maghreb at `7c7803a8` 1943.11: FRA 9 naval-base levels / 40 rail, GER 13 / 10).
...
- **A gauge whose condition closed keeps its last value forever, and the save cannot
  distinguish that from a live reading — read the family's `_last_t`, which `tlm` now puts
  on the value row.** GER on `9d83084c` reports `wa_tlm_r99_tunis_states_manned = 3` in
  1946.2 with `wa_tlm_r99_tunis_last_t = 84` (1943.1): three years after leaving Africa.
  `tlm` annotates any family lagging the save by more than **2 months** as
  `FROZEN Nmo (last sampled YYYY.M)` — 2 months because the standard sample is monthly and
  a save is a snapshot between two samples, so one month of lag is ordinary cadence and two
  is the first age cadence cannot explain. A `_last_t = 0` family reads `[never sampled]`.
- **Only a `_hist` array is a time series.** The ring-buffer suffix is `_hist`
  (`WA_TLM_TELEMETRY_SYSTEM.md`); everything else indexed — `wa_tlm_pc_built_by_type`,
  `wa_tlm_sq_adds_by_type`, `wa_tlm_pc_queued_t`, `wa_tlm_llr_sent_n` — is a scalar table
  keyed by the family's own key (a building type id, a project slot). `tlm` used to render
  those under the shared date axis, printing 450 railways built (`built_by_type^13`) as
  "1939.7 450". They are now printed as a `table` with raw indices, and a `_t` table decodes
  its **values** (clocks) instead.
...
Example — what priority construction was doing for GER as its railway queue collapsed, and how much civ industry it was eating (the `--match` filter here is a deliberate no-match, so only the per-save summaries plus the `NO MATCH` notice print):
