...
Dispatch map: `infantry_*.txt` → infantry (the dispatcher also names `special_forces_doctrine.txt`, deleted when the special-forces doctrine moved to `common/doctrines/`); `support*.txt` → support; `armor_*.txt` → armor; `air_techs*.txt`, `naval*.txt` / `MTG_naval.txt`, `industry.txt` / `electronic_mechanical_engineering*.txt` → legacy parsers (the dispatcher matches on `industry` / `electronic` substrings — `tools/migrations/ai_will_do/ai_will_do_replacer_all.py`).
